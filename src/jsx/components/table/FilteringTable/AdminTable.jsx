...
const AdminTable = ({ data, columns, title, leads, superAdmin, createNewLead, refetch, showFilter, crmFilter, makeRec, makeRecovery, pag, search }) => {
  ...
  return (
    <>
    <div className='' style={{flex: 1, height: '100%', color: "#F5EDED",  backgroundColor: '#2a2a2a', borderRadius: '10px', marginBottom: '20px'}}>
        {
          title && (
          <div className="card-header" style={{ display: "flex", justifyContent: "space-between" }}>
            <h4 className="card-title" style={{color: "#686D76"}}>{title}</h4>
            {
              leads && (
      ...
      <Card.Body className="" style={{ overflow: 'auto' }}>
          ...
          <div className="table-responsive">
            <table {...getTableProps()} style={{border: "none"}} className="table  display" >
              <thead style={{border: "none"}}>
                {headerGroups.map(headerGroup => (
                  <tr {...headerGroup.getHeaderGroupProps()}>
                    {headerGroup.headers.map(column => (
                      <th {...column.getHeaderProps()} style={{color: "#F5EDED", border: 'none'}}>
              ...
              <tbody {...getTableBodyProps()} style={{border: 'none'}}>
                {page.map((row) => {
                  prepareRow(row)
                  return (
                    <tr {...row.getRowProps()} style={{border: 'none', color: '#E2DAD6'}}>
